...
import {
  KPI_MASTER_TABLE,
  TRAINING_PLAN_TABLE,
  PROCESS_RECORDS_TABLE,
  SUPPLIER_CRITERIA_TABLE,
  QUALITY_POLICY_FORM,
  SMART_OBJECTIVE_FORM,
  JOB_DESCRIPTION_FORM,
  CLIENT_INTERVIEW_FORM,
  FODA_CROSS_MATRIX,
  COMPETENCY_MATRIX,
} from "./workspace/configs/iso9001Schemas.js";
  ...
  { id:"p2", phase:"Fase 2", title:"Mapeo de Procesos Clave", iconKey:"gears",
    ...
    sections:[
      ...
      { title:"Documentación por Proceso", items:[
        ...
        { text:"Diagrama de flujo para cada proceso", guide:"El diagrama de flujo muestra visualmente el paso a paso del proceso. Usen simbología estándar: rectángulo = actividad, rombo = decisión, óvalo = inicio/fin, flecha = flujo. Incluyan: quién ejecuta cada paso (usar swimlanes si son varios roles), puntos de decisión (ej: '¿Solicitud completa? Sí → continuar, No → devolver al asesor'), registros que se generan en cada punto. Herramientas: pueden hacerlos en LEGION, Mermaid, Lucidchart, o incluso en PowerPoint.", fieldType:"flowchart" },
                ...
                return <div key={ii} style={{borderBottom:ii<sec.items.length-1?`1px solid ${P.borderL}`:"none"}}>
                  ...
                  <div className="action-btns" style={{display:"flex",gap:6}}>
                    ...
                    <button className="togbtn" onClick={()=>setOpenWork(p=>({...p,[wk]:!p[wk]}))} style={{
                      ...
                      {I.edit(wOpen?"#92613a":P.s400)} Espacio de trabajo {filled?"·":""}
                      {filled&&<span style={{width:6,height:6,borderRadius:3,background:"#92613a",opacity:.6}}/>}
                    </button>
                  </div>
